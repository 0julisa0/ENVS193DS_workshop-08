...
shot mass and total mass are stronly positive  
root mass and total mass have a stronly positive correlated corr. is 0.858
...
biomass have different means, species on left have greater biomass means. gindelia
salvia, centranth,..... have the lowest mean total mass
```{r}
...
drought distressed species have lower levels of total biomass, weel watered species have higher levels of total biomass
```{r}
ggplot(data = drought_exp_clean, # data frame
...
there is a moderate positive relationship between leaf total biomass and specific leaf area
```{r}
ggplot(data = drought_exp_clean, # data frame
       aes(x = sla, # x-axis
           y = total_g)) + # y-axis
...
the saturated model is the species, water level, and specific leaf area 
...
model 1, specific leaf area, species, water treatment 
...
summary(model1)
```
(doesnt show up in list, therefore refrence level)
...
- intercept: **mass of Acmispon glaber in drought stressed treatment when SLA = 0**  mass of acispon glaber in drought stressed treatment when SLA=0 
...
- well watered estimate: **across species and SLA, well watered plants tend to be 0.12 +- (figure out how to write plus minus in text) 0.02 g heavier than drought stressed plants**    NO EFFECTS ON MASS
